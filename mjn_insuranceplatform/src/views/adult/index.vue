<template>
    <div class="adult">
        <div class="search">
          ...
          <div class="row">
            <div class="row-title">人生阶段</div>
            <div class="row-content" v-for="(item,index) in lifeStageList" :key="index">
              <input type="radio" :id="'lifeStage' + item.value" :value="item.value" v-model="lifeStage" />
						  <label :for="'lifeStage' + item.value">{{item.name}}</label>
            </div>
            <div class="row-main">
                <input type="text" v-model="insuranceAge" class="ivu-input" style="width:100px;margin-top: 14px;" placeholder="请输入周岁">
            </div>
          </div>
          <div class="row">
        ...
        <div class="compared" v-show="isCompared">
          ...
          <div class="content" v-for="(item,index) in itemsJSON" :key="index">
...
export default {
  ...
  watch: {
    ...
    itemsJSON: function() {
      var itemsJSON1 = JSON.parse(localStorage.getItem("itemsJSON"));
      if (itemsJSON1.length > 0) {
  ...
  methods: {
    ...
    getInitData() {
      ...
      // 表格数据
      tasks.push(
        this.$http(connectURL.adultTableData, {
          type: "GET",
          params: {
      ...
      // 条件数据
      tasks.push(this.$http(connectURL.adultConditionData));
      const task = Promise.all(tasks)
        .then(res => {
          let [adultTableData, adultConditionData] = res.map(item => {
            return item;
          });
          console.log(adultTableData);
          // 失败
          if (
            adultTableData.status != 200 &&
            adultConditionData.status != 200
          ) {
            this.$Message.warning(adultTableData.statusText);
            return;
          }
          // 设置总数
          this.total = adultTableData.data.total;
          adultTableData.data.rows.forEach((item, index) => {
            //设置对比状态
            item.isActive = false;
            //添加性价比
          ...
          if (localStorage.itemsJSON == "" || localStorage.itemsJSON == undefined ) {
          } else {
            this.itemsJSON = JSON.parse(localStorage.itemsJSON);
            this.itemsJSON.forEach((item, indexs) => {
              adultTableData.data.rows.forEach((items, index) => {
                if (item.id == items.id) {
                  items.isActive = true;
                }
              });
          ...
          // 设置表格内容
          this.tableContent = adultTableData.data.rows;
          // 设置搜索内容
          this.genderList = adultConditionData.data.gender; //性别
          this.lifeStageList = adultConditionData.data.lifeStage; //人生阶段
          this.insurancePeriodList = adultConditionData.data.insurancePeriod; //保障期限
          this.insuranceFeeList = adultConditionData.data.insuranceFee; //保费
          this.insuranceAmountList = adultConditionData.data.insuranceAmount; //保额
          this.socialSocietyList = adultConditionData.data.socialSociety; //有无社保
          this.insuranceTypeList = adultConditionData.data.insuranceType; //保障类型
          this.newinsuranceTypeList = this.insuranceTypeList;
          this.paymentMethodList = adultConditionData.data.paymentMethod; //付费方式
    ...
    handleContrast(params) {
      if (window.localStorage) {
        if (
          ...
          localStorage.itemsJSON == undefined
        ) {
          var array = [];
        ...
        if (params.row.isActive == false) {
          var itemsJSON1 = JSON.parse(localStorage.getItem("itemsJSON"));
          if (itemsJSON1.length > 3) {
            this.$Message.error("选择过多，只能对比四组数据");
            return false;
          } else {
            itemsJSON1.push({ name: params.row.name, id: params.row.id ,contrast_product_id : params.row.contrast_product_id});
            localStorage.setItem("itemsJSON", JSON.stringify(itemsJSON1));
            this.itemsJSON = JSON.parse(localStorage.itemsJSON);
            params.row.isActive = !params.row.isActive;
          }
        } else {
          var itemsJSON2 = JSON.parse(localStorage.itemsJSON);
          itemsJSON2.forEach((item, index) => {
            console.log(item.id, params.row.id);
            if (item.id == params.row.id) {
              console.log(item);
              itemsJSON2.splice(index, 1);
              localStorage.setItem("itemsJSON", JSON.stringify(itemsJSON2));
              this.itemsJSON = JSON.parse(localStorage.itemsJSON);
              params.row.isActive = !params.row.isActive;
            }
          });
    ...
    handlePageNumber(page) {
      ...
      if(this.insuranceAge == ""){
        ...
        var insuranceAge = this.insuranceAge + "Y";
      }
      this.$http(connectURL.adultTableData, {
        type: "GET",
        params: {
    ...
    cancelFun() {
      let removeLength = this.itemsJSON.length;
      this.itemsJSON.splice(0, removeLength);
      this.isCompared = false;
    ...
    comparedFun() {
      let array = [];
      this.itemsJSON.forEach((item, index) => {
